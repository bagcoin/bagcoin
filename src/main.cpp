...
#if defined(NDEBUG)
# error "Bagcoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nBits, int nHeight, const CAmount& nFees)
{
    ...
    // Subsidy is cut in half every 21429 blocks which will occur approximately every 1 months.
    nSubsidy >>= halvings;
...
bool IsInitialBlockDownload()
{
    ...
    if (lockIBDState)
        return false;
    //kevin bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
...
bool static LoadBlockIndexDB()
{
    ...
    LogPrintf("LoadBlockIndexDB(): hashBestChain=%s height=%d date=%s progress=%f nTime=%d nNone=%d\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(),
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
        Checkpoints::GuessVerificationProgress(chainActive.Tip()),
              chainActive.Tip()->GetBlockTime(),
              chainActive.Tip()->nNonce);
