...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "\nAvailable commands:\n"
                "  vote-many          - Vote on a Bagcoin initiative\n"
                "  vote-alias         - Vote on a Bagcoin initiative\n"
                "  vote               - Vote on a Bagcoin initiative/budget\n"
    ...
    if(strCommand == "vote-many")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage of vote-many is 'mnbudget vote-many PROPOSAL-NAME URL PAYMENT_COUNT BLOCK_START PLC_ADDRESS PLC_AMOUNT YES|NO|ABSTAIN'");
    ...
    if(strCommand == "vote")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage of vote-many is 'mnbudget vote PROPOSAL-NAME URL PAYMENT_COUNT BLOCK_START PLC_ADDRESS PLC_AMOUNT YES|NO|ABSTAIN'");
