...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS, bool fRequested)
{
    ...
    if(AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)){
        ...
        // should be at least not earlier than block when 1000 PLC tx got MASTERNODE_MIN_CONFIRMATIONS
        uint256 hashBlock = 0;
        CTransaction tx2;
        ...
        {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 PLC tx -> 1 confirmation
